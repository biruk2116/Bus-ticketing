    ...
    <div className="min-h-screen bg-slate-50 px-4 py-16 text-slate-900 transition-colors duration-300 dark:bg-slate-950 dark:text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-md">
        ...
        >
          ...
          <h1 className={`mt-4 text-4xl font-black ${darkMode ? 'text-white' : 'text-slate-950'}`}>Create your account</h1>
          <p className={`mt-3 ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>Set up a local demo account and continue through the booking flow.</p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            {[
              ...
              <div key={key}>
                <label htmlFor={key} className={`mb-2 block text-sm font-medium ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{label}</label>
                <div className="relative">
                  <Icon className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-sky-400" />
                  <input
                    id={key}
